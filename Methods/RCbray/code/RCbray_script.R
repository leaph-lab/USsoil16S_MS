...
raup_crick=function(spXsite, plot_names_in_col1=TRUE, classic_metric=FALSE, split_ties=TRUE, reps=999, set_all_species_equal=FALSE, as.distance.matrix=TRUE, report_similarity=FALSE){
...
com_m = read.csv("df_abun_trans_mod.csv")

rc = raup_crick(com_m)
x = as.matrix(rc)
m = reshape2::melt(x, na.rm=T)
names(m) <- c("Sample ID_y", "Sample ID_x", "RCbray")

write.csv(m, "RCbray_Results_abun.csv")
